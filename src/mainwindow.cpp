	...
	QMainWindow(parent), mainwindow_ui(new Ui::MainWindow)
{
	...
	m_main_widget = new MainWidget(m_data_container, input_form, &m_vis_settings, related_widgets, &m_selection_boxes, mainwindow_ui->centralwidget);
...
void MainWindow::initializeColorCodeCheckBox()
{
	...
	color_coding_checkbox->setChecked(false);
...
void MainWindow::on_vis_clicked()
{
	...
	foreach(const auto & id, m_selection_boxes.keys())
	{
		m_selection_boxes[id]->setStyleSheet("border: 1px solid gray; font-size: 12pt");
	...
	sender->setStyleSheet("border: 1px solid orange; font-size: 12pt");
}
